...
Runs persistent vector retrieval + NetworkX graph traversal in parallel, merges context, and evaluates router accuracy across 15 questions.
"""
...
class VectorStore:
    ...
    def add(self, document: str, doc_id: str, metadata: Dict[str, Any]):
        vec = self._embed(document)
...
# Populate Vector Collection with Document Chunks
DOC_CHUNKS = [
...
for i, doc in enumerate(DOC_CHUNKS):
    vector_store.add(document=doc, doc_id=f"doc_{i}", metadata={"source": f"chunk_{i}"})
